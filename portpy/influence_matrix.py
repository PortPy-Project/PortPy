...
class InfluenceMatrix:
    ...
    - **Attributes** ::
        ...
        :param dose_3d: 3D dose distribution. It is saved as attribute once the method dose_1d_to_3d() is called
                so that it doesnt have to be computed again

    - **Methods** ::
        :dose_1d_to_3d(sol)
            Convert dose from 1d to 3d and return dose in 3d
        :dose_3d_to_1d(dose_3d)
            Convert dose from 3d to 1d voxels and return dose in 1d
    ...
    def __init__(self, plan_obj,
        ...
        if not self.is_sparse:
            ...
            self.A_full = self.get_influence_matrix(plan_obj, self.is_sparse)  # create full matrix
        self.dose_3d = None
        self._vox_map = None
    ...
    def dose_1d_to_3d(self, sol: dict = None, dose_1d: np.array = None) -> np.ndarray:
        """
        Create 3d dose from dose in 1d voxels. 3d array have same resolution as CT
    ...
    def dose_3d_to_1d(self, dose_3d: np.ndarray) -> np.array:
        """
        Get dose in 1d voxels for the given influence matrix from 3d dose
    ...
    def down_sample_voxels(self, down_sample_xyz: List[int] = None):
        """
        This method creates new ct to dose voxel map array based on down sampled voxels.
    ...
    def pre_process_voxels(self, plan_obj):
        """
        Updates opt_voxels_dict based upon ct to dose voxel map
