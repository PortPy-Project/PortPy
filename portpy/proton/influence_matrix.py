...
from portpy.proton.beams import Beams
...
class InfluenceMatrix:
    ...
    def __init__(self, structs: Structures, beams: Beams,
        ...
        if 'spots' in beams.beams_dict:
            self.beamlets_dict = deepcopy(beams.beams_dict['spots'])
        else:
            self.beamlets_dict = deepcopy(self.beamlets_dict['spots'])
    ...
    def get_influence_matrix(self, is_full=False):
        ...
        if not is_full:
            ...
            inf_matrix_full = deepcopy(self._beams.beams_dict['influenceMatrixFull'])
    ...
    def preprocess_beams(self, structure='PTV', remove_corner_beamlets=False, is_bev=False):
        ...
        for ind in range(len(self.beamlets_dict)):
            ...
            if not is_bev:
                ...
                spots = self.beamlets_dict[ind]

            if ind > 0:
                temp_opt_beamlets = np.unique(spots['id'][0]) + self.beamlets_dict[ind-1]['end_beamlet_idx'] + 1
            else:
                temp_opt_beamlets = np.unique(spots['id'][0])

            self.beamlets_dict[ind]['start_beamlet_idx'] = np.min(temp_opt_beamlets)
            self.beamlets_dict[ind]['end_beamlet_idx'] = np.amax(temp_opt_beamlets)
            self.beamlets_dict[ind]['opt_beamlets_ids'] = spots['id'][0]
